...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.id == undefined) {
      app.maskToast(that, 'http://wjdh.yccjb.com/errorIcon.png', '获取营销员编号失败', 1500);
    } else {
      wx.showLoading({
        title: '加载中',
      })
      wx.request({
        url: app.globalData.ipPath + '/index.php/api/SalesMan/salesManCommission',
        data: {
          id: options.id,
        },
        method: 'POST',
        ...
        success: function (res) {
          ...
          if (res.data.code == 1) {
            ...
            // app.maskToast(that, 'http://wjdh.yccjb.com/errorIcon.png', '获取代理商佣金规则失败', 1500);
          } else {
